...
#elif defined(DISCFERRET)			// Configuration bits for DiscFerret
...
{
	_asm
	bsf		TRISJ, 5, 0			//Address: 0x00		//Configure RJ5 as input pin (not strictly necessary since reset state is already = 1)
...
BootEntryIOCheck:
	...
	//Currently using RJ5 I/O pin for this purpose.  If jumper shorted (RJ5 = 0), enter bootloader firmware.
	//If not shorted (RJ5 = 1, due to pull up resistor), enter the main application firmware instead.
    btfss	PORTJ, 5, 0			//Address: 0x0C		//Check RB4 I/O pin state	
